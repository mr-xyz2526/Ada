...
const EXERCISES = [
  ...
  {
    id: "barbell_curl", name: "Barbell Curl", icon: "💪", focus: "Biceps",
    script: "barbell_curl_analyzer.py",
    cameraHint: "SIDE VIEW · camera at waist/hip height · full upper body (hip → wrist) visible",
    metrics: ["Elbow joint angle (shoulder→elbow→wrist)", "Elbow swing angle (forward drift)", "Torso sway angle (backward lean)", "Wrist break angle"],
    feedbacks: [
      { label: "ELBOW SWINGING", color: "#e05555", desc: "Pin elbows to your torso" },
      { label: "BODY ROCKING", color: "#e05555", desc: "Stand upright, no momentum" },
      { label: "WRIST HYPEREXTENSION", color: "#e8a83a", desc: "Keep wrists neutral" },
      { label: "INCOMPLETE EXTENSION", color: "#5b7cf5", desc: "Fully lower the bar each rep" },
      { label: "INCOMPLETE CONTRACTION", color: "#5b7cf5", desc: "Curl all the way to the top" },
    ],
    tips: ["SIDE VIEW camera at waist/hip height", "Elbows pinned to your torso throughout", "Full range: hang at bottom → contract at top", "No swinging — let biceps do all the work"],
    mode: "reps", color: "#f4a261", video: "https://www.youtube.com/watch?v=ykJmrZ5v0Oo"
  },
  {
    id: "bench_press", name: "Bench Press", icon: "🏋️", focus: "Chest, Shoulders & Triceps",
    script: "bench_press_analyzer.py",
    cameraHint: "SIDE VIEW · camera at bench height · full upper body (shoulder → wrist) visible",
    metrics: ["Elbow joint angle (shoulder→elbow→wrist)", "Upper arm vertical angle", "Wrist stack over elbow", "ROM (bar to chest)"],
    feedbacks: [
      { label: "BAR DRIFTING TO FACE", color: "#e05555", desc: "Elbows too far forward" },
      { label: "BAR DRIFTING TO BELLY", color: "#e05555", desc: "Elbows too far back" },
      { label: "WRIST NOT STACKED", color: "#e8a83a", desc: "Stack wrists over elbows" },
      { label: "INCOMPLETE ROM", color: "#5b7cf5", desc: "Lower bar to chest each rep" },
      { label: "MISSING LOCKOUT", color: "#00b894", desc: "Fully extend arms at the top" },
    ],
    tips: ["SIDE VIEW camera at bench height perpendicular to body", "Bar path straight up and down — avoid drift", "Touch or brush chest at the bottom", "Stack wrists directly over elbows"],
    mode: "reps", color: "#e76f51", video: "https://www.youtube.com/watch?v=vcBig73ojpE"
  },
  {
    id: "cable_fly", name: "Cable Fly", icon: "🦅", focus: "Chest (Pecs)",
    script: "cable_fly_analyzer.py",
    cameraHint: "FRONT VIEW · camera facing you directly · stand between cable stacks · full body visible",
    metrics: ["Wrist separation (shoulder-width units)", "Fly angle per arm (hip→shoulder→wrist)", "Elbow bend consistency", "Arm asymmetry", "Shoulder elevation"],
    feedbacks: [
      { label: "ELBOW OVER-BENT", color: "#e05555", desc: "Keep a soft, fixed elbow bend" },
      { label: "ELBOW INCONSISTENCY", color: "#e8a83a", desc: "Lock elbow angle mid-rep" },
      { label: "ARM ASYMMETRY", color: "#e8a83a", desc: "Both arms move together" },
      { label: "SHOULDER SHRUG", color: "#9b59b6", desc: "Depress shoulders, use chest" },
      { label: "INCOMPLETE STRETCH", color: "#5b7cf5", desc: "Open arms wide at the start" },
      { label: "INCOMPLETE SQUEEZE", color: "#5b7cf5", desc: "Fully contract pecs at peak" },
      { label: "TRUNK INSTABILITY", color: "#00b4d8", desc: "Stay tall — no swinging" },
    ],
    tips: ["FRONT VIEW camera — facing you directly", "Maintain a soft, constant elbow bend throughout", "Open wide at start, squeeze hard at peak contraction", "Both arms should move symmetrically"],
    mode: "reps", color: "#2ec4b6", video: "https://www.youtube.com/watch?v=taI4XduLpTk"
  },
  {
    id: "chest_dips", name: "Chest Dips", icon: "⬇️", focus: "Chest & Triceps",
    script: "chest_dips_analyzer.py",
    cameraHint: "SIDE VIEW · camera at hip/waist height · full body in frame",
    metrics: ["Elbow angle (depth indicator)", "Torso lean angle (20–40° for chest)", "Hip swing from baseline", "Shoulder elevation", "Elbow flare"],
    feedbacks: [
      { label: "ELBOW FLARE", color: "#e05555", desc: "Keep elbows back, not out" },
      { label: "SHALLOW DEPTH", color: "#e8a83a", desc: "Descend to 90° elbow angle" },
      { label: "INCOMPLETE LOCKOUT", color: "#5b7cf5", desc: "Press fully up each rep" },
      { label: "EXCESSIVE LEAN", color: "#e05555", desc: "Lean 20–40° for chest focus" },
      { label: "HIP/KNEE SWING", color: "#e8a83a", desc: "No kipping — chest only" },
      { label: "SHOULDER ELEVATION", color: "#9b59b6", desc: "Depress shoulders at top" },
    ],
    tips: ["SIDE VIEW camera at hip/waist height", "Lean forward 20–40° to target chest over triceps", "Lower until elbows hit ~90°, press fully back up", "Keep elbows tracking back, not flaring out"],
    mode: "reps", color: "#c77dff", video: "https://www.youtube.com/watch?v=2z8JmcrW-As"
  },
  {
    id: "incline_db_press", name: "Incline DB Press", icon: "📐", focus: "Upper Chest & Shoulders",
    script: "incline_db_press_analyzer.py",
    cameraHint: "SIDE VIEW · camera at bench/shoulder height · full upper body + arms visible",
    metrics: ["Elbow angle (shoulder→elbow→wrist)", "Shoulder angle (hip→shoulder→elbow)", "Elbow flare from torso", "Wrist alignment over elbow"],
    feedbacks: [
      { label: "ELBOW FLARE", color: "#e05555", desc: "Elbows 45–60° from torso" },
      { label: "INCOMPLETE REP", color: "#5b7cf5", desc: "Lower to full stretch each rep" },
      { label: "ELBOWS LOCKED", color: "#e8a83a", desc: "Slight bend at top, don't hyperextend" },
      { label: "WRIST CAVING", color: "#e8a83a", desc: "Keep wrists stacked over elbows" },
    ],
    tips: ["SIDE VIEW camera at bench/shoulder height", "Bench set at 30–45° incline", "Lower dumbbells until full stretch, press in an arc inward", "Keep elbows at 45–60° from torso throughout"],
    mode: "reps", color: "#f77f00", video: "https://www.youtube.com/watch?v=DbFgADa2PL8"
  },
  {
    id: "tricep_pushdown", name: "Tricep Pushdown", icon: "⬇️", focus: "Triceps",
    script: "tricep_push_down_analyzer.py",
    cameraHint: "SIDE VIEW · camera at waist/hip height · full upper body in frame",
    metrics: ["Elbow angle (extension indicator)", "Upper arm vertical angle (drift)", "Torso lean angle", "Shoulder elevation", "Elbow flare"],
    feedbacks: [
      { label: "UPPER ARM SWINGING", color: "#e05555", desc: "Pin elbows to your sides" },
      { label: "EXCESSIVE LEAN", color: "#e8a83a", desc: "Slight lean only — max 20°" },
      { label: "SHOULDER SHRUG", color: "#9b59b6", desc: "Keep shoulders depressed" },
      { label: "ELBOW FLARE", color: "#e8a83a", desc: "Elbows stay parallel, don't flare" },
      { label: "WRIST DEVIATION", color: "#5b7cf5", desc: "Keep wrists neutral throughout" },
      { label: "INCOMPLETE LOCKOUT", color: "#00b894", desc: "Fully extend arms at the bottom" },
    ],
    tips: ["SIDE VIEW camera at waist/hip height", "Upper arms must stay pinned vertically to your torso", "Full extension at bottom, controlled return to start", "Slight forward lean (5–20°) is fine — no excessive lean"],
    mode: "reps", color: "#7b2d8b", video: "https://www.youtube.com/watch?v=2-LAMcpzODU"
  },
];
...
export default function ARTrainer({ user, T, initialExId = null, onClearEx = () => {} }) {
  ...
  const startARSession = async (exId) => {
    setCameraError(null);
    setIsActivating(true);
    try {
      const res = await fetch(`http://localhost:5000/start/${exId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mode: 'beginner', speed: 5, mute: false })
      });
      ...
      if (data.success) {
        ...
        setCameraError(data.message || "Failed to start AR session");
      }
    } catch (err) {
      setCameraError("Could not connect to AR Bridge Server. Make sure it's running.");
    } finally {
  ...
  useEffect(() => {
    if (initialExId) {
      ...
      if (found) {
        setSelected(found);
        // Auto-activate if server is online
        if (serverStatus === "online") {
          startARSession(found.id);
        }
        onClearEx();
      }
    }
  }, [initialExId, onClearEx, serverStatus]);
          ...
          <div style={{ background: "#0a0a0a", borderRadius: 12, position: "relative", height: 440, overflow: "hidden", border: `2px solid ${active ? selected.color : T.border}`, transition: "border-color .3s" }}>
            {active ? (
              <img 
                ...
                alt="AR Feed"
                onError={() => {
                  setCameraError("Video feed lost. Check if AR Server is still running.");
                  setActive(false);
                }}
              />
            ) : null}
            ...
            {!active && (
              <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, padding: 20 }}>
                ...
                {serverStatus === "offline" && (
                  <div style={{ background: "rgba(224,85,85,0.1)", border: "1px solid rgba(224,85,85,0.3)", borderRadius: 8, padding: "8px 14px", fontSize: 11, color: "#e05555", textAlign: "center", maxWidth: 280 }}>
                    ⚠️ <strong>AR Bridge Offline</strong><br/>
                    Run <code>python ar_server.py</code> to enable AR
                  </div>
                )}
                
                <button 
                  onClick={() => startARSession(selected.id)} 
                  disabled={isActivating || serverStatus === "offline"}
                  style={{ 
                    background: (isActivating || serverStatus === "offline") ? "rgba(255,255,255,0.08)" : selected.color, 
                    color: "#fff", 
                    border: "none", 
                    padding: "12px 40px", 
                    borderRadius: 24, 
                    fontWeight: 700, 
                    cursor: (isActivating || serverStatus === "offline") ? "not-allowed" : "pointer", 
                    fontFamily: "DM Sans", 
                    fontSize: 14, 
                    display: "flex", 
                    alignItems: "center", 
                    gap: 8, 
                    transition: "all .2s",
                    opacity: serverStatus === "offline" ? 0.5 : 1
                  }}
                >
                  {isActivating
                    ? <><span style={{ width: 14, height: 14, border: "2px solid rgba(255,255,255,0.3)", borderTopColor: "#fff", borderRadius: "50%", animation: "spin 0.8s linear infinite", display: "inline-block" }} /> Activating...</>
                    : "📷 Activate AR"}
                </button>
              </div>
            )}
            ...
            {active && (
              <>
                ...
                <div style={{ position: "absolute", top: 20, left: 20, background: "rgba(0,0,0,0.78)", padding: "6px 12px", borderRadius: 7, backdropFilter: "blur(8px)" }}>
                  <div style={{ fontSize: 9, fontWeight: 700, color: selected.color, letterSpacing: ".1em", textTransform: "uppercase" }}>● LIVE AR</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#fff", marginTop: 2 }}>{selected.name.toUpperCase()}</div>
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", marginTop: 1 }}>AI Pose Detection Feed</div>
                </div>
                ...
                <button onClick={stopARSession} style={{ position: "absolute", top: 16, right: 16, background: "rgba(224,85,85,0.85)", color: "#fff", border: "none", width: 34, height: 34, borderRadius: "50%", cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)" }}>✕</button>
              </>
            )}
          </div>
